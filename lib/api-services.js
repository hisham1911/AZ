...
export async function uploadExcelFile(file) {
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        throw new Error(
          "الملف غير صالح. يرجى التحقق من تنسيق الملف والمحاولة مرة أخرى."
        );
      } else if (response.status === 401 || response.status === 403) {
        throw new Error("ليس لديك صلاحية لرفع ملفات الشهادات.");
      } else if (response.status === 500) {
        throw new Error(
          "خطأ في الخادم. يرجى المحاولة مرة أخرى لاحقًا أو الاتصال بالدعم."
        );
      } else {
    ...
    // إبطال مفعول التخزين المؤقت بعد الرفع
    cache.invalidate(/^(getAllServices|searchByName|searchBySerial)/);

    return result;
  } catch (error) {
    // إذا كان الخطأ مُعالج بالفعل، نمرره، وإلا نعطي رسالة عامة
    if (error.message.includes("Failed to fetch")) {
      throw new Error(
